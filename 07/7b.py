...
card_mappings = {'A': 14, 'K': 13, 'Q': 12, 'T': 10,
...
class Hand:
    def __init__(self, one, two, three, four, five) -> None:
        ...
        if 1 in self.counts and self.counts[1] and self.counts[1] != 5:  # jokers
            num_jokers = self.counts[1]
            del self.counts[1]
            for i in self.counts.keys():
                if i != 1 and self.counts[i] == max(self.counts.values()):
                    self.counts[i] += num_jokers
